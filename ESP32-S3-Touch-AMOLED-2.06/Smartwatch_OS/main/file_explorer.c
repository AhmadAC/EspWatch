...
void init_es8311_codec(uint32_t sample_rate, uint16_t bits_per_sample) {
    ...
    uint8_t reg01 = 0x3F; // MCLK from MCLK pin, enable all clocks
    es8311_write_reg(0x01, reg01);
    ...
    es8311_write_reg(0x0D, 0x01); // Power up analog circuitry - NOT default
    vTaskDelay(pdMS_TO_TICKS(10));
    es8311_write_reg(0x0E, 0x02); // Enable analog PGA, enable ADC modulator - NOT default
    vTaskDelay(pdMS_TO_TICKS(10));
    es8311_write_reg(0x12, 0x00); // power-up DAC - NOT default
    es8311_write_reg(0x13, 0x10); // Enable output to HP drive - NOT default
    es8311_write_reg(0x1C, 0x6A); // ADC Equalizer bypass, cancel DC offset in digital domain
    es8311_write_reg(0x37, 0x08); // Bypass DAC equalizer - NOT default
    
    // Volume & Unmute
    es8311_write_reg(0x31, 0x00); // Unmute DAC output
    es8311_write_reg(0x32, 0xFF); // Set Speaker Volume to Max 
